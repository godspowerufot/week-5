...
async function airdropSOL(solanaConnection, recipientPubKey, solAmount) {
    if(solAmount > 0) {
        ...
        if (res.status !== 200) {
            ...
            console.warn("Airdrop request failed:", res.status, errorText);
            return;
        }
        
        const responseData = await res.json();
        console.log("Airdrop signature:", responseData.result);
        
        // Fixed polling logic - wait UNTIL balance is enough
        let accountBalance = BigInt(0);
        let attempts = 0;
        const maxAttempts = 30;
        
        while(accountBalance < solAmount && attempts < maxAttempts) {
            await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second
            accountBalance = await solanaConnection.getBalance(recipientPubKey);
            attempts++;
        }
        
        if (accountBalance >= solAmount) {
            console.log("Airdrop successful! New balance:", ethers.formatUnits(accountBalance.toString(), 9), "SOL");
        } else {
            console.warn("Timeout waiting for airdrop confirmation");
        }
    }
}
